...
{	
	...
	public static void main (String[] x){
		...
		String commandLine = "ffmpeg -i c:\\video\\video1.avi -b 200 -r 30 -s 320x240 -sameq -ss -hq -deinterlace -ab 16 -f flv -acodec mp3 -ar 22050 -ac 2 -y c:\\video\\felipe.flv ";
		...
		try {
			...
			int seconds = (Integer.parseInt(time[0])*3600)+(Integer.parseInt(time[1])*60) + (Integer.parseInt(time[2])*1);
			
			System.out.println("TEMPO:" + seconds);
			
			int i = 1 + (int)(Math.random() * seconds) ;
			String generateThumbnail="ffmpeg -itsoffset -"+ i +"  -i c:\\video\\video1.avi -vcodec mjpeg -vframes 1 -an -f rawvideo -s 320x240 -y c:\\video\\video1.jpg";
			cmdExecutor.runCommand(generateThumbnail);
			System.out.println(generateThumbnail);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if(cmdError != null && !"".equals(cmdError))
